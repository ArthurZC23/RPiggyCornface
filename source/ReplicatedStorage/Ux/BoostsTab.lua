    ...
    return function()
        ...
        if ActiveV and not conn then
            conn = ActiveV.AncestryChanged:Connect(function(_, parent)
                conn:Disconnect()
                conn = nil
                Dilatation.shrink(imageBtn, tabSize)
            end)
        else
            Dilatation.shrink(imageBtn, tabSize)
        end
...
function module.onHover(imageBtn, data, tabSize, kwargs)
    return function()
        imageBtn.ImageColor3 = data.colors.hover
        Dilatation.expand(imageBtn, tabSize, kwargs.dilatation)
    end
end

-- kwargs allow for custom behavior
function module.addUx(tab, btn, productData, kwargs)
    local tabSize = btn.Size
    ...
    --     btn.SelectionGained:Connect(module.onHover(btn, productData, tabSize, kwargs))
    -- else
    --     btn.MouseLeave:Connect(module.onLeft(tab, btn, productData, tabSize, kwargs))
    --     btn.MouseEnter:Connect(module.onHover(btn, productData, tabSize, kwargs))
    -- end
    btn.MouseLeave:Connect(module.onLeft(tab, btn, productData, tabSize, kwargs))
    btn.MouseEnter:Connect(module.onHover(btn, productData, tabSize, kwargs))
