...
local CharsDistancesS = {}
CharsDistancesS.__index = CharsDistancesS
CharsDistancesS.className = "CharsDistances"
CharsDistancesS.TAG_NAME = CharsDistancesS.className

function CharsDistancesS.new()
    local self = {
        distances = {},
    }
    setmetatable(self, CharsDistancesS)
...
function CharsDistancesS:updateDistances()
    local playersData = {}
    self.distances = {}
    local idx = 1
...
function CharsDistancesS:getCharsWithDistLTE(char, distThreshold, conditions)
...
function CharsDistancesS:getClosestChar(char)
    if not (self.distances[char] and next(self.distances[char])) then return end
    if not (self.distances[char]["array"] and next(self.distances[char]["array"])) then return end
    table.sort(self.distances[char]["array"], function(a, b) return a.distance < b.distance end)
    return self.distances[char]["array"][1].char, self.distances[char]["array"][1].distance
end

function CharsDistancesS:areCharsNear(char1, char2, distance)
...
function CharsDistancesS:Destroy()
    self._maid:Destroy()
end

SingletonsManager.addSingleton(CharsDistancesS.className, CharsDistancesS.new())

return CharsDistancesS