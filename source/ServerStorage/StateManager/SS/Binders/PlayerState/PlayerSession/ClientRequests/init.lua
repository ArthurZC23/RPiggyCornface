...
function ClientRequests.new(playerSession)
    local self = {}
    ...
    self.SetRequestPlayerSessionRF = self.player.Remotes.Functions.SetRequestPlayerSession

    self.SetRequestPlayerSessionRF.OnServerInvoke = function(plr, scope, action)
...
function ClientRequests:handleRequest(scope, action)
    ...
    if handler and (not self.handlerDbs[scope][action.name]) then
        ...
        local ok, err = pcall(function()
            return handler[action.name](self.playerSession, scope, action)
        end)
