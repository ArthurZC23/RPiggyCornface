...
local NpcCoreAnimationsR15 = {}
NpcCoreAnimationsR15.__index = NpcCoreAnimationsR15
NpcCoreAnimationsR15.className = "NpcCoreAnimationsR15"
NpcCoreAnimationsR15.TAG_NAME = NpcCoreAnimationsR15.className

function NpcCoreAnimationsR15.new(char)
    local self = {
        char = char,
        _maid = Maid.new(),
        lastTick = os.clock(),
    ...
    setmetatable(self, NpcCoreAnimationsR15)

    if not self:getFields() then return end

    self.CoreAnimationsData = Data.Animations[("CoreAnimationsR15")]
...
function NpcCoreAnimationsR15:setParameters()
...
function NpcCoreAnimationsR15:getRigScale()
...
function NpcCoreAnimationsR15:preloadCoreAnimations()
    local coreAnimations = self.CoreAnimationsData.defaultCoreAnimations
    if self.char:GetAttribute("CoreAnimations") == "Monster" then
        coreAnimations = self.CoreAnimationsData.monsterCoreAnimations
    end
...
function NpcCoreAnimationsR15:getPoseData(pose)
...
function NpcCoreAnimationsR15:getPoseAnimSignal(pose)
    return self[("%sUpdateSE"):format(pose)]
end

function NpcCoreAnimationsR15:setPoseAnimations(pose, poseStyle, animationsData, kwargs)
...
function NpcCoreAnimationsR15:removePoseAnimation(pose, animationId, kwargs)
    local poseAnims = self.animationsFolder[pose]
...
function NpcCoreAnimationsR15:setInitialPoseToAnims()
    if self.char:GetAttribute("CoreAnimations") == "Monster" then
        self.poseToAnims = TableUtils.deepCopy(self.CoreAnimationsData.monsterCoreAnimations)
        self.poseStyle = {}
...
function NpcCoreAnimationsR15:createSignals()
...
function NpcCoreAnimationsR15:clearCoreAnimations()
...
function NpcCoreAnimationsR15:createAnimationsFolder()
    ...
    self.animationsFolder = Data.Animations.Animations[("CoreR15")][coreAnimations]:Clone()
    self.animationsFolder.Name = "CoreAnimations"
    self.animationsFolder.Parent = self.char
end

function NpcCoreAnimationsR15:getFields()
...
function NpcCoreAnimationsR15:configureAnimationSet(poseName, animationsData)
...
function NpcCoreAnimationsR15:configureAllAnimationSet()
...
function NpcCoreAnimationsR15:setRunSpeed(speed)
...
function NpcCoreAnimationsR15:setAnimationSpeed(speed)
...
function NpcCoreAnimationsR15:stopAllAnimations()
...
function NpcCoreAnimationsR15:getHeightScale()
...
function NpcCoreAnimationsR15:rootMotionCompensation(speed)
...
function NpcCoreAnimationsR15:keyFrameReachedFunc(frameName)
...
function NpcCoreAnimationsR15:_sampleAnimation(animName)
    -- TableUtils.print(self.animTable)
    -- print(animName)
...
-- Preload animations
function NpcCoreAnimationsR15:playAnimation(animName, transitionTime)
...
function NpcCoreAnimationsR15:toolKeyFrameReachedFunc(frameName)
	if (frameName == "End") then
...
function NpcCoreAnimationsR15:playToolAnimation(animName, transitionTime, priority)
    local newAnimation = self:_sampleAnimation(animName)
...
function NpcCoreAnimationsR15:stopToolAnimations()
...
function NpcCoreAnimationsR15:onRunning(speed)
...
function NpcCoreAnimationsR15:onDied()
	self.pose = "Dead"
end

function NpcCoreAnimationsR15:onJumping()
	self:playAnimation("jump", 0.1)
	self.jumpAnimTime = self.jumpAnimDuration
	self.pose = "Jumping"
end

function NpcCoreAnimationsR15:onClimbing(speed)
	if self.userAnimateScaleRun then
...
function NpcCoreAnimationsR15:onGettingUp()
	self.pose = "GettingUp"
end

function NpcCoreAnimationsR15:onFreeFall()
...
function NpcCoreAnimationsR15:onFallingDown()
	self.pose = "FallingDown"
end

function NpcCoreAnimationsR15:onSeated()
	self.pose = "Seated"
end

function NpcCoreAnimationsR15:onPlatformStanding()
	self.pose = "PlatformStanding"
end

function NpcCoreAnimationsR15:onSwimming(speed)
	if self.userAnimateScaleRun then
		speed /= self:getHeightScale()
	end
	if speed > 1.00 then
...
function NpcCoreAnimationsR15:getTool()
    return self.char:FindFirstChildOfClass("Tool")
end

function NpcCoreAnimationsR15:getToolAnim(tool)
...
function NpcCoreAnimationsR15:animateTool()
...
function NpcCoreAnimationsR15:move(timestamp)
...
function NpcCoreAnimationsR15:setHumanoidEvents()
...
function NpcCoreAnimationsR15:Destroy()
    self._maid:Destroy()
    self.isBinded = false
    self:clearCoreAnimations()
end

return NpcCoreAnimationsR15