...
local function _hasGp()
    -- local gpsState = playerState:get(S.Stores, "GamePasses")
    -- local vipGpId = Data.GamePasses.GamePasses.nameToData[S.VipGp].id
    -- if not gpsState[vipGpId] then return end
    return false
end
local function _hasRolePermission()
    local GroupRoles = Data.Players.Roles.GroupRoles.GroupRoles
    local Policies = Data.Players.Policies.Policies
    local role = LocalPlayer:GetAttribute("groupRole")
    print(role)
    local policies = GroupRoles.roles[role].policies
    return policies[Policies.policies.freeCam.name]
end
local function hasFreeCamPermission()
    return _hasGp() or _hasRolePermission()
end
...
do
	...
	local function HandleActivationInput(action, state, input)
		if state == Enum.UserInputState.Begin and hasFreeCamPermission() then
