...
local ChangeMonsterSkinPage = SharedSherlock:find({"EzRef", "Get"}, {inst=gui, refName="ChangeMonsterSkinPage"})
local SkinProto = SharedSherlock:find({"EzRef", "Get"}, {inst=ChangeMonsterSkinPage, refName="SkinProto"})
local SkinContainer = SkinProto.Parent
SkinProto.Parent = nil

local View = {}
View.__index = View
View.className = "ChangeMonsterSkinView"
...
function View:setSkinShopReward(morphId, _gui, data, shopData)
    ...
    if shopData._type == "Money" then
        ...
        -- Price.Price.Size = UDim2.fromScale(0.3, 1)
    ...
    elseif shopData._type == "Gamepass" then
        ...
        Price.Visible = true
    elseif shopData._type == "NotShopReward" then
        Price.ImageLabel:Destroy()
        -- Price.Price.Size = UDim2.fromScale(1, 1)
        Price.Price.TextXAlignment = Enum.TextXAlignment.Center
        Price.Price.TextYAlignment = Enum.TextYAlignment.Center
        Price.Price.Text = ("%s"):format(shopData.viewRewardType)
        Price.Visible = true
    end
...
function View:shouldShowMorph(id, data)
    return true
end
...
function View:createSkinGui()
    ...
    local function update()
        ...
        for id, data in pairs(Data.MonsterSkins.MonsterSkins.idData) do
            ...
            if not self:shouldShowMorph(id, data) then continue end
...
local TabPage = SharedSherlock:find({"EzRef", "Get"}, {inst=gui, refName="ChangeMonsterSkinPage"})
